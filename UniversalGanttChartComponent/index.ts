    ...
    implements ComponentFramework.StandardControl<IInputs, IOutputs>
{
    ...
    constructor() {
        ...
        this.handleExpanderStateChange =
            this.handleExpanderStateChange.bind(this);
    ...
    private async updateViewAsync(
        ...
        const startField = columns.find(
            (c) => c.alias === this._scheduledStartStr
        );
        const endField = columns.find((c) => c.alias === this._scheduledEndStr);
        const progressField = columns.find(
            (c) => c.alias === this._progressStr
        );
        if (
            !nameField ||
            !startField ||
        ...
        try {
            ...
            const recordDisplayName =
                context.parameters.customHeaderDisplayName.raw ||
                nameField.displayName;
            const startDisplayName =
                context.parameters.customHeaderStartName.raw ||
                startField.displayName;
            const endDisplayName =
                context.parameters.customHeaderEndName.raw ||
                endField.displayName;
            ...
            //width setup
            const columnWidthQuarter =
                context.parameters.columnWidthQuarter.raw || 0;
            ...
            const columnWidthMonth =
                context.parameters.columnWidthMonth.raw || 0;
    ...
    private async generateTasks(
        ...
        for (const recordId of dataset.sortedRecordIds) {
            ...
            const taskTypeOption = <string>(
                record.getValue(this._taskTypeOption)
            );
            ...
            const optionValue = <string>(
                record.getValue(this._displayColorOption)
            );
                ...
                if (parentRecord) {
                    ...
                    if (parentRecordRef) {
                        const parentType = this.getTaskType(
                            <string>(
                                parentRecordRef.getValue(this._taskTypeOption)
                            ),
                            context.parameters.taskTypeMapping.raw
                        );
